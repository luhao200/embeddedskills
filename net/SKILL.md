...
- 脚本目录: `~/.claude/skills/net/scripts/`
- 配置文件: `~/.claude/skills/net/config.json`
- 协议参考: `~/.claude/skills/net/references/common_protocols.json`
...
```bash
python ~/.claude/skills/net/scripts/net_iface.py [--filter <关键词>] [--tshark] [--json]
```
...
```bash
python ~/.claude/skills/net/scripts/net_capture.py [--output <文件路径>] [--format <pcapng|pcap>] [--decode-as <规则>] [--json]
```
...
```bash
python ~/.claude/skills/net/scripts/net_analyze.py <pcap_file> [--mode <summary|protocols|conversations|endpoints|io|anomalies|all>] [--filter <显示过滤器>] [--top <数量>] [--decode-as <规则>] [--export-fields <字段列表>] [--output <CSV路径>] [--json]
```
...
```bash
python ~/.claude/skills/net/scripts/net_ping.py [--tcp <端口>] [--count <次数>] [--traceroute] [--concurrent <线程数>] [--json]
```
...
```bash
python ~/.claude/skills/net/scripts/net_scan.py [--timeout <毫秒>] [--banner] [--concurrent <线程数>] [--json]
```
...
```bash
python ~/.claude/skills/net/scripts/net_stats.py [--interval <秒>] [--mode <overview|protocol|endpoint|port>] [--json]
```
